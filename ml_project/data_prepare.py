...
def data_prepare(input_data_path, split):
    ...
    logging.info(f'Data from {input_data_path} prepared and splitted to train and test parts ({train_size:4.2f} and {test_size:4.2f} respectively)')
    
    train_df.to_csv(train_dataset_path, index=False)
    test_df.to_csv(test_dataset_path, index=False)

    logging.info(f'Train part was saved to {train_dataset_path}')
    logging.info(f'Test part was saved to {test_dataset_path}')
